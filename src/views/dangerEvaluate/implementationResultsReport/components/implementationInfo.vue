<template>
  <div>
    ...
    <!-- 查看风险 -->
    <implementation-dialog ref="implementationDialog" :dataSource="dataSource"></implementation-dialog>
  </div>
</template>
...
import implementationDialog from "./implementationDialog";
export default {
  components: { taskDetail, implementationDialog },
  mixins: [initData],
  data() {
    return {
      dataSource: []
  ...
  methods: {
    showRiskList(row) {
      ...
      let _this = this.$refs.implementationDialog;
      _this.dialog = true;
    },
    showRiskMeasure(row) {
      this.dataSource = [
