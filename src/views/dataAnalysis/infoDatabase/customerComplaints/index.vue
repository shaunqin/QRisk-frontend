<template>
  <div class="app-container">
    <div class="head-container">
      <search sourceType="info_source_customer" type="8" />
      <el-button
        class="filter-item"
        size="mini"
        type="primary"
    ...
    >
      ...
      <el-table-column :label="$t('global.operation')" width="150" fixed="right">
        <template slot-scope="{row}">
          ...
          <el-button v-if="row.status=='0'&&row.updData" type="primary" size="mini" icon="el-icon-upload" @click="doSub(row.id)"></el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
    ...
    <editform ref="editform" type="8" :is-add="isAdd" :isSubmit="isSubmit"></editform>
  </div>
</template>
