<template>
  ...
  <el-card>
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button v-if="step == 2 || step == 6 || step == 8" :loading="loading" type="primary" @click="doSubmit">确认</el-button>
    </div>
    <!-- 抄送 -->
      ...
      specialRiskComplete({ ...this.form, ...params }).then((res) => {
        if (res.code != "200") {
          ...
          if(this.$parent.$parent.$parent.step == 1) {
            this.$parent.$parent.$parent.$parent.subHandle(obj)
          } else {
            this.$parent.$parent.$parent.$parent.$parent.subHandle(obj)
          }
        }
        this.loading = false;
      });
