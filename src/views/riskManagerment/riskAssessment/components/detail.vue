  ...
  >
    <div v-if="assessmentType=='4'">
      <el-form size="mini" label-width="auto">
        <el-form-item label="标题" prop="data.title">{{data.title}}</el-form-item>
        <el-row :gutter="16">
          <el-col :span="12">
          ...
          <el-col :span="12">
            ...
            <el-form-item label="截止日期" prop="data.endTime">{{data.endTime}}</el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <el-table :data="list" size="mini" :span-method="objectSpanMethod" border height="550">
        ...
        <el-table-column label="控制措施" min-width="200">
          <template slot-scope="{row}">{{row.specialRiskMeasureList[0].controlMeasure}}</template>
        </el-table-column>
        <el-table-column label="责任单位" width="160">
          <template slot-scope="{row}">{{row.specialRiskMeasureList[0].reponsibleDept}}</template>
        </el-table-column>
        <el-table-column label="控制状态" width="110">
          <template slot-scope="{row}">{{row.specialRiskMeasureList[0].completion}}</template>
        </el-table-column>
      </el-table>
    </div>
    <div v-else>
