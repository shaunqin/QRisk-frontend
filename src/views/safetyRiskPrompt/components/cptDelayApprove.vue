...
<script>
import { riskNoticeDelayApproveList, riskNoticeDelayApprove } from "@/api/risk";
...
export default {
  ...
  methods: {
    ...
    loadTable() {
      if (this.id) {
        this.loading = true;
        riskNoticeDelayApproveList(this.id).then(res => {
          this.loading = false;
          if (res.code != '200') {
    ...
    doSubmit() {
      ...
      riskNoticeDelayApprove(this.form).then(res => {
